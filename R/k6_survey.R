...
##### k6-survey and save as csv file
...
for (id in id_list) {
  ...
  # import json file
  data_list <- read_json(
    paste0("data/expfactory/", id, "_finished/k6-survey-results.json"))
  
  # convert list to tibble
  tbl <- as_tibble(data_list)
  
  # move variable name into one column and values into another
  tbl <- gather(tbl, key = varname, value = value)
  
  # edit variable name in column 1
  ...
  # move variable from varname to separate columns
  tbl <- tbl %>%
    spread(varname, value)
  
  # add subject id
  tbl <- tbl %>%
    mutate(id = id)# load libraries
...
# save as csv file
write_csv(x = dataset, path = "k6-survey-results-all.csv")
